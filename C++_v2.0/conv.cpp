...
void conv(float *input, float *weights, float* tmp, float *output, int *params, int *sparams){
	...
	float weight_buf[outCh_once][1024];
	float in_buf[64][outCh_once*img_num];
    float tmp_buf[outCh_once][img_num];
  	...
  	for(int o = 0; o<iteration_out; o++)
   	{
    	for(int i = 0; i<iteration_in; i++)
    	{
       		...
       		for(int p = 0; p<outCh_once; p++)
			{
				...
				int weight_offset = (((o*outCh_once+p))*inChannel+i*inCh_once)*ksize*ksize;
			...
			for(int y = 0; y < y_out; y++)
				for(int x = 0; x < x_out; x++)
				{
					...
					for(int p = 0; p<outCh_once; p++)
					{
						int tmp_offset = ((y*x_out+x)*outCh_once+p)*img_num;
						...
						if(i == iteration_in - 1)
						{
							for(int im = 0; im<img_num; im++)     
							{
								int out_offset = ((y*x_out+x)*outChannel+o*outCh_once+p)*img_num+im;
...
void conv_back(float *outdiff, float *weights, float *indiff, float *tmp, int *params, int *sparams)
{
	...
	float weight_buf[inCh_once][1024];
	float out_buf[64][img_num];
  	float in_buf[64][inCh_once][img_num];
				...
				for(int x = 0; x < x_out; x++)
    	 		{
        			...
        			for(int p=0;p<ksize;p++)
          				for(int q=0;q<ksize;q++)
        				{
							...
							{
							int in_off = ((y_in*x_dim+x_in) * inCh_once)*img_num;
					...
					for(int m = 0; m<ksize; m++)
						for(int n = 0; n<ksize; n++)
						{
							...
							int y_in = y*stride-pad+m;
          					...
          					if(x_in<0 || y_in<0 || x_in>x_dim-1 || y_in>y_dim-1)
          						continue;
							int in_trans_size = inCh_once;
							int tmp_offset = (y_in * x_dim+x_in)*inCh_once*img_num;
							int in_offset =  ((y_in * x_dim+x_in)*inChannel+i*inCh_once)*img_num;
            				...
            				{ 
								...
								{ 
       	    					...
       	    					memcpy(tmp + tmp_offset, in_buf[m*ksize+n], inCh_once*img_num*4);
          				}
